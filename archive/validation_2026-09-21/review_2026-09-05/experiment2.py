...
def find_project_root(start: Optional[Path] = None) -> Path:
    """Locate the project root whether the notebook is run from root or NonStationary/."""
    current = Path(start or Path.cwd()).resolve()
    if current.is_file():
        current = current.parent
    for candidate in (current, *current.parents):
        if (candidate / 'NonStationary' / 'experiment2.ipynb').exists() and (candidate / 'experiment.ipynb').exists():
            return candidate
        if candidate.name == 'NonStationary' and (candidate / 'experiment2.ipynb').exists():
            return candidate.parent
    return current.parent if current.name == 'NonStationary' else current


PROJECT_ROOT = find_project_root()
NONSTATIONARY_DIR = PROJECT_ROOT / 'NonStationary'
NS_RESULTS_DIR = NONSTATIONARY_DIR / 'Results'
NS_RESULTS_DIR.mkdir(parents=True, exist_ok=True)
...
def ns_path(*parts) -> Path:
    return NONSTATIONARY_DIR.joinpath(*parts)


def ns_results_path(*parts) -> Path:
    return NS_RESULTS_DIR.joinpath(*parts)


def resolve_uber_parquet_dir(preferred: Optional[Path] = None) -> Path:
    """Resolve Uber parquet data without hard-coding one user's absolute path."""
    candidates = []
    if preferred is not None:
        candidates.append(Path(preferred).expanduser())
    env_path = os.environ.get('UBER_PARQUET_DIR')
    if env_path:
        candidates.append(Path(env_path).expanduser())
    candidates.extend([
        PROJECT_ROOT.parent / 'Uber_NYC',
        PROJECT_ROOT / 'Uber_NYC',
        NONSTATIONARY_DIR / 'Uber_NYC',
    ])
    unique_candidates = []
...
UBER_PARQUET_DIR = resolve_uber_parquet_dir()
HOURLY_ALPHA_CACHE = ns_results_path('hourly_alpha_2021.csv')


def fit_hourly_alpha_from_parquet(parquet_dir: Path, cache_path: Path = HOURLY_ALPHA_CACHE) -> np.ndarray:
    # Compute alpha(t): average hourly trip ratio across days, with project-local caching.
    ...
    if not files:
        raise FileNotFoundError(
            ...
            'Set UBER_PARQUET_DIR or place Uber_NYC next to the project root, inside the project root, or inside NonStationary/.'
        )
...
print('Uber parquet dir:', UBER_PARQUET_DIR)
print('Hourly alpha cache:', HOURLY_ALPHA_CACHE)
...
class CoModalEnv:
    ...
    - Observation: vector with normalized features:
        * pos (x/R, y/R), time_frac, cyclic hour-of-day (sin, cos), flags (to_pickup, with_pass)
    ...
    def _visible_rides(self) -> List[RideReq]:
        ...
        for (r, ttl) in self.ride_buffer:
            ...
            if dist_to_pickup > self.r_pick:
                continue
            # 2) TTL + ETA filtering: must be able to reach pickup from that reference before TTL expires.
    ...
    def _get_obs(self) -> Tuple[np.ndarray, np.ndarray]:
        ...
        hour_frac = (self.t % 1440.0) / 1440.0
        # Cyclic hour-of-day features expose the 24-hour demand cycle without a midnight discontinuity.
        hour_sin = math.sin(2.0 * math.pi * hour_frac)
        hour_cos = math.cos(2.0 * math.pi * hour_frac)
        ...
        core = [self.pos[0] / self.R, self.pos[1] / self.R, time_frac, hour_sin, hour_cos] + flags + counts
    ...
    @property
    def obs_dim(self):
        # core (x, y, time_frac, hour_sin, hour_cos + 2 flags + 2 counts) + 3*K + 6*MAX_VISIBLE
        return (5 + 2 + 2) + 3 * self.k_pack + 6 * self.max_visible
...
def baseline_four_zone(env: CoModalEnv,
    ...
    Four-Zone Policy (FZ):
      ...
      3) In the "final leg" within a zone, open a passenger screening window (lasting screen_window_min minutes):
           Only consider visible rides where pickup is within r_pick of the delivery-reference point, TTL is valid, and dropoff is in an "unserved" zone;
    ...
    def screened_visible_in_unserved():
        ...
        for r in visible:
            ...
            if zid in unserved_zids:
                # env._visible_rides already filters pickup radius and TTL from the delivery-location reference.
                cand.append(r)
        # Select the one nearest to the same visibility reference.
        vis_ref = env._visibility_reference_point()
        cand.sort(key=lambda rr: manhattan(vis_ref, rr.pickup))
        return cand
    ...
    def _forced_nearest():
        if current_zone is not None and idx_in_route < len(route):
            return np.array(route[idx_in_route], dtype=np.float32)
        return None
    env._nearest_package = _forced_nearest
    screen_left = 0
...
import os, csv
TRAIN_LOG_PATH = ns_results_path('training_log.csv')
...
def run_param_sweep(
    ...
    *,
    ...
    if csv_path is None:
        csv_path = ns_results_path("param_sweep_results.csv")
    rows = []
    combo_id = 0
...
def plot_from_csv(csv_path: str, vary_keys, metrics=("rate", "reward", "accepted")):
    ...
    for k in vary_keys:
        for m in metrics:
            out = ns_results_path(f"plot_{m}_vs_{k}.png")
...
R_PICK_ALPHA_list      = [0.1,0.2,0.3,0.4]
...
CSV_PATH = ns_results_path("param_sweep_results_2.csv")
...
df = run_param_sweep(LAMBDA_list,
                     ...
                     SWITCH_GRACE_STEPS_list,
                     RT_list,
                     train_updates_per_combo=40,      # Set to 20~50 for short DRL training. Set to 200 for full training of each parameter combination.
                     csv_path=CSV_PATH,
                     seed_offset=0)
...
plot_rate_vs_lambda_optimal_alpha_by_gamma(
    ...
    gamma_values=(0.33, 0.5, 0.67, 0.83, 1),
    algos=("DRL", "HEUR", "HEUR_VOR", "PURE", "PURE_OR"),
    savepath_prefix=ns_path("plot_rate_vs_lambda_optimal_alpha")
)
...
plot_rate_vs_gamma_by_params(
    ...
    max_subplots_per_row=3,
    savepath=ns_results_path("plot_rate_vs_gamma_by_params.png")  # Optional: save the figure
)
