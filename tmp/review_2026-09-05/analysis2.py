...
def find_project_root(start: Path | None = None) -> Path:
    """Locate the project root whether this notebook is run from root or NonStationary/."""
    current = Path(start or Path.cwd()).resolve()
    if current.is_file():
        current = current.parent
    for candidate in (current, *current.parents):
        if (candidate / 'NonStationary' / 'analysis2.ipynb').exists() and (candidate / 'experiment.ipynb').exists():
            return candidate
        if candidate.name == 'NonStationary' and (candidate / 'analysis2.ipynb').exists():
...
# ---------- Global Parameters ----------
CSV_PATH = ns_results_path("param_sweep_results_2.csv") #For the full parameter test with lambda=40 and all alpha, use param_sweep_results_2_full_param.csv
...
PANELS = [
    {"title": r"$n=30,\ r_\ell=6$", "gamma": 0.50, "filename": ns_path("rev_rate_vs_lambda_n30.png")},
    {"title": r"$n=50,\ r_\ell=6$", "gamma": 0.83, "filename": ns_path("rev_rate_vs_lambda_n50.png")},
]
...
for lam in LAMBDAS:
    ...
    # Save figure
    filename = ns_path(f"rev_rate_vs_n_lambda{lam}.png")
...
# Save figure
filename = ns_path("ratio_hvor_drl_vs_lambda.png")
fig.savefig(filename, dpi=300, bbox_inches="tight")
print(f"Saved: {filename}")
...
# Save figure
filename = ns_path("ratio_hvor_drl_vs_n.png")
fig.savefig(filename, dpi=300, bbox_inches="tight")
print(f"Saved: {filename}")
...
log_path = ns_results_path('training_log.csv')
if not log_path.exists():
    raise FileNotFoundError('NonStationary/Results/training_log.csv not found. Run experiment2.ipynb after the fixed logger changes to generate a fresh log.')
